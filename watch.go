...
func watchLoop(opts options) {
	...
	for {
		...
		renderWatch(opts, tick, cachedRows, errMsg, refreshStatus(lastFetch, nextRefresh), t, showURLs)
...
func renderWatch(opts options, tick int, rows []Row, errMsg, status string, t terminal, showURLs bool) {
	fmt.Print("\033[H")
	p := colors(t.useColor)
	...
	default:
		...
		out += renderTerminal(filtered, t.width, t.useColor, showURLs)
	}
