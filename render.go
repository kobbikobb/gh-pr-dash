...
func renderTerminal(rows []Row, width int, color bool, mergedTotal int) string {
	...
	const prefixW = 1 + 3 + 1 + 2 + 8 + 2 + idleW + 2 // indent + num(3) + ci + merge(8) + idle(idleW) + double-spaces
	...
	for i, r := range rows {
		...
		tierNum++
		num := p.d + padRight(strconv.Itoa(tierNum)+".", 3) + p.z
		...
		fmt.Fprintf(&b, " %s  %s  %s  %s  %s  %s   %s\n",
			num, ciGlyph(r.CI, p), merge, idle, repo, title, url)
	}
